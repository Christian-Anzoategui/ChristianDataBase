...
//cout << "data_count = " << it->data_count << endl;
		if(key_ptr+1 == it->data_count){
//cout << "key_ptr = " << key_ptr << endl;
    ...
    void copy_tree(const BPlusTree<T>& other);  //shell to allow function
    void copy_tree(const BPlusTree<T>& other, stack<BPlusTree<T>* > &s, int &link);//copy other
...
*/
	if(!other.is_leaf() )
	{
		...
		for(int i=0; i<child_count; ++i)
		{
			...
			subset[i]->copy_tree(*other.subset[i], s, link);
...
if(debug)cout<<"this: "<<this<<endl;
if(debug)cout<<"link count: "<<link<<endl;
		s.push(this);
		++link;
	}
